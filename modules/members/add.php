...
<div class="space-y-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 border-t-8 border-kebana-blue shadow-sm">
        <div>
            ...
            <p class="text-xs font-black text-slate-500 uppercase tracking-widest mt-2">Sila masukkan maklumat lengkap ahli baru.</p>
        </div>
    ...
    <div class="p-6 <?php echo $message_type === 'success' ? 'bg-green-50 border-green-600 text-green-800' : 'bg-red-50 border-red-600 text-red-800'; ?> border-l-4 font-black text-sm uppercase tracking-widest shadow-sm">
    ...
    <div class="bg-white border-t-8 border-kebana-yellow shadow-sm p-8 md:p-12 space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs font-black text-slate-500 uppercase tracking-widest mt-1">Imbas borang bercetak menggunakan kamera telefon atau fail gambar untuk mengisi maklumat secara automatik.</p>
            </div>
        ...
        <div class="border-2 border-dashed border-slate-300 hover:border-kebana-blue bg-slate-50/50 hover:bg-slate-50 transition-all p-8 flex flex-col items-center justify-center text-center cursor-pointer relative group" id="ocr_dropzone">
            ...
            <span class="text-xs text-slate-500 font-bold uppercase tracking-wide mt-1 block">Format: JPG, PNG, WEBP (Maksima 5MB)</span>
        </div>

        <!-- Continue on Mobile Button -->
        <div class="flex justify-center pt-2">
            <button type="button" id="btn_continue_mobile" class="w-full sm:w-auto px-8 py-4 bg-kebana-blue text-white text-xs font-black uppercase tracking-[0.2em] hover:bg-kebana-accent transition-colors flex items-center justify-center gap-3 shadow-md">
        ...
        <div id="ocr_process_state" class="hidden bg-slate-50 p-6 border-l-4 border-kebana-blue space-y-4">
            <div class="flex items-center justify-between">
                <span id="ocr_status_text" class="text-xs font-black text-kebana-blue uppercase tracking-widest">Memuatkan Enjin OCR...</span>
            ...
            <form method="POST" class="space-y-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="md:col-span-2">
                        <label for="full_name" class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-4">NAMA PENUH (SEPERTI DALAM IC)</label>
                        <input type="text" id="full_name" name="full_name" required 
                               class="w-full px-6 py-5 bg-slate-50 border-b-2 border-slate-200 focus:border-kebana-blue focus:bg-white outline-none text-sm font-bold tracking-tight transition-all rounded-none"
                    ...
                    <!-- Gender -->
                    <div>
                        <label for="gender" class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-4">JANTINA</label>
                        <select id="gender" name="gender" required 
                                class="w-full px-6 py-5 bg-slate-50 border-b-2 border-slate-200 focus:border-kebana-blue focus:bg-white outline-none text-sm font-bold transition-all rounded-none appearance-none">
                    ...
                    <!-- IC Number -->
                    <div>
                        <label for="ic_number" class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-4">NO. KAD PENGENALAN</label>
                        <input type="text" id="ic_number" name="ic_number" required 
                               class="w-full px-6 py-5 bg-slate-50 border-b-2 border-slate-200 focus:border-kebana-blue focus:bg-white outline-none text-sm font-bold transition-all rounded-none"
                    ...
                    <!-- Phone Number -->
                    <div>
                        <label for="phone_no" class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-4">NO. TELEFON</label>
                    ...
                    <!-- Village -->
                    <div>
                        <label for="village" class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-4">KAWASAN / KAMPUNG</label>
                    ...
                    <!-- Status -->
                    <div>
                        <label for="status" class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-4">STATUS KEAHLIAN</label>
                ...
                <div class="pt-10 flex flex-col md:flex-row gap-6">
                    <button type="submit" class="flex-1 bg-kebana-blue text-white py-6 text-sm font-black uppercase tracking-[0.3em] hover:bg-kebana-accent transition-all shadow-xl">
                        SIMPAN MAKLUMAT AHLI
                    </button>
                    <button type="reset" class="px-10 py-6 border-2 border-slate-200 text-slate-400 text-sm font-black uppercase tracking-widest hover:bg-slate-50 transition-all">
                        SET SEMULA
                    </button>
                </div>
            </form>
    ...
    <div class="bg-slate-50 p-8 border border-slate-200">
        <div class="flex items-start space-x-6">
            ...
            <div>
                <h4 class="text-sm font-black text-kebana-blue uppercase tracking-widest">Nota Penting</h4>
                <p class="text-xs text-slate-500 font-bold uppercase mt-2 leading-relaxed tracking-tight">
...
<div id="mobile_ocr_modal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm hidden flex items-center justify-center z-[9999] p-4 transition-all duration-300">
    <div class="bg-white border-t-8 border-kebana-blue shadow-2xl w-full max-w-md p-8 md:p-10 relative space-y-6 text-center transform scale-95 transition-transform duration-300">
        <div>
            ...
            <p class="text-xs font-black text-slate-500 uppercase tracking-widest mt-1">Imbas kod QR untuk memulakan pengimbasan dokumen.</p>
        </div>
        
        <!-- QR Code Container -->
        <div class="flex flex-col items-center justify-center space-y-4">
            <div id="qr_code_container" class="p-4 bg-white border border-slate-200 shadow-inner flex items-center justify-center" style="min-height: 200px; min-width: 200px;">
                <i class="fa-solid fa-spinner fa-spin text-3xl text-kebana-blue/20"></i>
            </div>
            
            <div class="flex items-center justify-center gap-3 text-kebana-blue font-black text-xs uppercase tracking-widest" id="polling_status">
                <span class="w-2.5 h-2.5 rounded-full bg-kebana-yellow animate-pulse"></span>
                <span>Menunggu sambungan telefon...</span>
            </div>
        </div>

        <div class="text-xs text-slate-500 font-bold uppercase tracking-tight leading-relaxed text-left max-w-xs mx-auto space-y-1">
        ...
        <button type="button" id="btn_close_mobile_ocr" class="w-full py-4 border-2 border-slate-200 text-slate-400 text-xs font-black uppercase tracking-widest hover:bg-slate-50 transition-all">
            BATALKAN SAMBUNGAN
        </button>
    </div>
</div>
