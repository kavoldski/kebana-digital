...
<body class="min-h-full flex flex-col font-sans antialiased text-slate-900">
    ...
    <main class="flex-1 bg-[#F8FAFC] min-h-screen pt-12 pb-32">
        <div class="max-w-4xl mx-auto px-6">
            ...
            <article class="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden p-8 sm:p-16 space-y-12">
                ...
                <?php if (!empty($images)): ?>
                    <div class="relative rounded-[2rem] overflow-hidden shadow-2xl bg-black aspect-video group" id="announcementCarousel">
                        <!-- Slides wrapper -->
                        <div class="flex h-full w-full" id="carouselSlides">
                            <?php foreach ($images as $index => $img): ?>
                                <div class="w-full h-full flex-shrink-0 relative overflow-hidden carousel-slide-item">
                        ...
                        <?php if (count($images) > 1): ?>
                            <button type="button" id="prevBtn" class="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-2xl glass-nav hover:bg-kebana-blue hover:text-white flex items-center justify-center text-slate-800 transition-all shadow-lg border border-white/20 active:scale-95 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <!-- Dot indicators -->
                            <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center space-x-2.5 bg-black/35 backdrop-blur-md px-4 py-2.5 rounded-full border border-white/10" id="dotsContainer">
                                <?php foreach ($images as $index => $img): ?>
                                    <button type="button" data-slide-index="<?php echo $index; ?>" 
                                            class="carousel-dot h-2.5 rounded-full transition-all duration-300 bg-white/60 hover:bg-white"></button>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                    </div>
    ...
    <!-- Carousel Javascript (Only active if there are images) -->
    <?php if (count($images) > 1): ?>
    <script>
