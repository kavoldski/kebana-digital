...
<div class="space-y-12">
    ...
    <div class="flex flex-col md:flex-row md:items-start justify-between gap-6 bg-white p-8 border-t-8 border-kebana-blue shadow-sm">
        <div class="min-w-0">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <span class="px-3 py-1 text-[8px] font-black uppercase tracking-widest <?php echo ($event['event_level'] ?? 'MASTER') === 'MASTER' ? 'bg-kebana-blue text-white' : 'bg-slate-100 text-slate-500'; ?>">
            ...
            <div class="flex items-center gap-4 mt-3 flex-wrap">
                ...
                ?>
                <span class="px-3 py-1 text-[8px] font-black uppercase tracking-widest <?php echo $s_cls; ?>"><?php echo $ev_status; ?></span>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-0 border border-slate-100 bg-white">
        <div class="p-8 border-r border-slate-50 flex flex-col justify-center">
            ...
            <?php if ($budget_est > 0): ?>
            <p class="text-2xl font-black text-kebana-blue mt-3">RM <?php echo number_format($budget_est, 2); ?></p>
            ...
            <?php else: ?>
            <p class="text-lg font-black text-slate-300 mt-3">Tiada Bajet</p>
            <?php endif; ?>
        </div>
        <div class="p-8 border-r border-slate-50 flex flex-col justify-center">
            ...
            <p class="text-[9px] font-black mt-2 uppercase tracking-widest <?php echo $variance >= 0 ? 'text-green-500' : 'text-red-500'; ?>">
        ...
        <div class="p-8 border-r border-slate-50 flex flex-col justify-center">
            ...
            <p class="text-[9px] font-black text-slate-400 mt-2 uppercase"><?php echo count(array_filter($transactions, fn($t) => $t['trans_type']==='Income')); ?> rekod masuk</p>
        </div>
        <div class="p-8 flex flex-col justify-center border-b-8 <?php echo $net_pl >= 0 ? 'border-green-500' : 'border-red-500'; ?>">
            <p class="text-[9px] font-black uppercase tracking-widest <?php echo $net_pl >= 0 ? 'text-green-400/70' : 'text-red-400/70'; ?>">UNTUNG / RUGI BERSIH</p>
            <p class="text-2xl font-black mt-3 <?php echo $net_pl >= 0 ? 'text-green-600' : 'text-red-600'; ?>">
                <?php echo ($net_pl >= 0 ? '+' : '') . 'RM ' . number_format($net_pl, 2); ?>
            </p>
            <p class="text-[8px] font-black mt-2 uppercase tracking-wider <?php echo $net_pl >= 0 ? 'text-green-400' : 'text-red-400'; ?>">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-0 border border-slate-100 bg-white shadow-sm overflow-hidden">
        ...
        <div class="lg:col-span-3 border-r border-slate-100">
            <div class="p-8 border-b border-slate-50 bg-slate-50/50">
                ...
                <p class="text-[9px] font-black text-slate-400 uppercase tracking-tighter mt-1">Baki kumulatif mengikut urutan transaksi</p>
            </div>
        ...
        <div class="lg:col-span-2">
            <div class="p-8 border-b border-slate-50 bg-slate-50/50">
                ...
                <p class="text-[9px] font-black text-slate-400 uppercase tracking-tighter mt-1">Mengikut kategori</p>
            </div>
            <div class="p-8 flex items-center justify-center" style="height: 280px;">
                <?php if (!empty($cat_breakdown)): ?>
                <canvas id="catChart"></canvas>
                <?php else: ?>
                <p class="text-[10px] font-black text-slate-300 uppercase tracking-widest text-center">Tiada perbelanjaan direkodkan</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/50">
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Tarikh</th>
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Jenis</th>
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Kategori</th>
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Mod</th>
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Resit</th>
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Amaun</th>
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Direkod Oleh</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    <?php if (empty($transactions)): ?>
                    <tr>
                        <td colspan="7" class="px-8 py-20 text-center text-[10px] font-black text-slate-200 uppercase tracking-[0.3em]">
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-8 py-5">
                                <span class="px-3 py-1 text-[8px] font-black uppercase tracking-widest <?php echo $is_income ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'; ?>">
                            ...
                            <td class="px-8 py-5 text-center">
                                ...
                                <?php else: ?>
                                <span class="text-slate-200 text-[9px] font-black uppercase">—</span>
                                <?php endif; ?>
                            </td>
                ...
                <tfoot class="border-t-2 border-slate-100">
                    <tr class="bg-slate-50/50">
                        <td colspan="5" class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Jumlah Keseluruhan</td>
                        <td class="px-8 py-5 text-right">
                            <p class="text-sm font-black text-green-600">+ RM <?php echo number_format($total_income, 2); ?></p>
                            <p class="text-sm font-black text-red-600">− RM <?php echo number_format($total_expense, 2); ?></p>
                            <div class="border-t border-slate-200 mt-2 pt-2">
                                <p class="text-sm font-black <?php echo $net_pl >= 0 ? 'text-green-700' : 'text-red-700'; ?>">
                                    <?php echo ($net_pl >= 0 ? '+' : '') . 'RM ' . number_format($net_pl, 2); ?>
                                </p>
                            </div>
                        </td>
            ...
            <a href="<?= URL_ROOT ?>/finance/event/<?php echo $sub['event_id']; ?>"
               class="flex items-center justify-between px-8 py-5 hover:bg-slate-50 transition-colors group">
                <div>
                    ...
                    <p class="text-[9px] font-bold text-slate-400 uppercase mt-1"><?php echo htmlspecialchars($sub['cawangan_name'] ?? 'Cawangan'); ?> &nbsp;·&nbsp; <?php echo !empty($sub['event_date']) ? date('d M Y', strtotime($sub['event_date'])) : 'TBA'; ?></p>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    ?>
                    <span class="px-3 py-1 text-[8px] font-black uppercase tracking-widest <?php echo $ss_cls; ?>"><?php echo $s_status; ?></span>
                    <i class="fa-solid fa-arrow-right text-slate-300 group-hover:text-kebana-blue transition-colors text-sm"></i>
                </div>
            </a>
            <?php endforeach; ?>
        </div>
    ...
    if (balCtx) {
        ...
        new Chart(balCtx, {
            ...
            options: {
                ...
                scales: {
                    x: { grid: { display: false }, ticks: { font: { weight:'700' }, maxTicksLimit: 8 } },
                    y: { grid: { color: '#f1f5f9' }, ticks: { font: { weight:'700' }, callback: v => 'RM ' + v.toLocaleString('ms-MY') } }
                }
            }
        });
    }
    ...
    if (catCtx) {
        new Chart(catCtx, {
            ...
            options: {
                ...
                cutout: '65%',
                plugins: {
                    legend: { position: 'bottom', labels: { font: { weight:'700', size: 10 }, padding: 12, boxWidth: 12 } },
