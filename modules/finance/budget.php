    ...
    <div class="bg-white p-8 border border-slate-100 shadow-sm">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            <div>
                ...
                <select name="year" class="w-full px-5 py-4 bg-slate-50 border-b-2 border-slate-100 focus:border-kebana-blue focus:bg-white outline-none text-xs font-bold uppercase transition-all rounded-none appearance-none">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="<?php echo htmlspecialchars($filters['search']); ?>"
                           ...
                           class="w-full pl-12 pr-5 py-4 bg-slate-50 border-b-2 border-slate-100 focus:border-kebana-blue focus:bg-white outline-none text-xs font-bold uppercase transition-all">
                </div>
            </div>
    ...
    <div class="bg-white border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
            <div>
                ...
                <p class="text-[9px] font-black text-slate-400 uppercase tracking-tighter mt-1">Perbandingan visual setiap program — Tahun <?php echo $filters['year']; ?></p>
            </div>
        </div>
    ...
    <div class="bg-white border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4 text-[9px] font-black uppercase tracking-widest">
                ...
                <span class="flex items-center gap-1"><span class="w-2 h-2 bg-slate-200 inline-block"></span> Tiada Bajet</span>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/50">
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Program</th>
                        <th class="px-6 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Bajet</th>
                        <th class="px-6 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Belanja</th>
                        <th class="px-6 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Pendapatan</th>
                        <th class="px-6 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">P&L</th>
                        <th class="px-8 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest">Penggunaan Bajet</th>
                        <th class="px-6 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Status</th>
                        <th class="px-6 py-5 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    <?php if (empty($masters)): ?>
                    <tr>
                        <td colspan="8" class="px-8 py-20 text-center text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">
                        ...
                        <tr class="hover:bg-slate-50/30 transition-colors group bg-white">
                            <td class="px-8 py-5">
                                <div class="flex items-start gap-3">
                                    <span class="mt-0.5 px-2 py-0.5 text-[7px] font-black uppercase tracking-widest bg-kebana-blue text-white shrink-0"><?php echo $level; ?></span>
                                    <div>
                                        <p class="text-xs font-black text-kebana-blue uppercase tracking-tight"><?php echo htmlspecialchars($b['event_title']); ?></p>
                                        <p class="text-[9px] font-bold text-slate-400 uppercase mt-0.5 italic">
                            ...
                            <td class="px-6 py-5 text-right">
                                <p class="text-xs font-black text-slate-600 uppercase tracking-widest">
                                    <?php echo $hasbudget ? 'RM ' . number_format($b['planned_budget'], 2) : '<span class="text-slate-300 text-[9px]">—</span>'; ?>
                                </p>
                            </td>
                            <td class="px-6 py-5 text-right">
                                <p class="text-xs font-black text-red-600 uppercase tracking-widest">
                            ...
                            <td class="px-6 py-5 text-right">
                                <p class="text-xs font-black uppercase tracking-widest <?php echo $net_pl >= 0 ? 'text-green-600' : 'text-red-600'; ?>">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors group bg-slate-50/30">
                            <td class="px-8 py-4 pl-16">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="px-1.5 py-0.5 text-[7px] font-black uppercase tracking-widest bg-slate-100 text-slate-500 shrink-0">SUB</span>
                                    <div>
                                        <p class="text-[11px] font-bold text-slate-600 uppercase tracking-tight group-hover:text-kebana-blue transition-colors"><?php echo htmlspecialchars($s['event_title']); ?></p>
                                        <p class="text-[8px] font-bold text-slate-300 uppercase mt-0.5">
                            ...
                            <td class="px-6 py-4 text-right">
                                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                    <?php echo $sHasBud ? 'RM ' . number_format($s['planned_budget'], 2) : '<span class="text-slate-300">—</span>'; ?>
                                </p>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <p class="text-[10px] font-bold text-red-500 uppercase tracking-widest">RM <?php echo number_format($s['actual_expense'], 2); ?></p>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <p class="text-[10px] font-bold text-green-500 uppercase tracking-widest">RM <?php echo number_format($s['actual_income'], 2); ?></p>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <p class="text-[10px] font-bold uppercase tracking-widest <?php echo $s_pl >= 0 ? 'text-green-500' : 'text-red-500'; ?>">
                            ...
                            <td class="px-8 py-4">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <p class="text-[8px] font-black text-slate-400 uppercase"><?php echo $sUtil; ?>%</p>
                                </div>
                                <?php else: ?>
