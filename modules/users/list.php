...
<div class="space-y-12">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 border-t-8 border-kebana-blue shadow-sm">
        <div>
            ...
            <p class="text-xs font-black text-slate-500 uppercase tracking-widest mt-2">Urus akses dan peranan pengguna sistem KEBANA.</p>
        </div>
        <div class="flex gap-4">
            <a href="<?= URL_ROOT ?>/users/add" class="bg-kebana-blue text-white px-10 py-4 text-sm font-black uppercase tracking-[0.2em] hover:bg-kebana-accent transition-all shadow-xl inline-flex items-center">
    ...
    <div class="p-6 <?php echo $message_type === 'success' ? 'bg-green-50 border-green-600 text-green-800' : 'bg-red-50 border-red-600 text-red-800'; ?> border-l-4 font-black text-sm uppercase tracking-widest shadow-sm">
        <div class="flex items-center justify-between">
            ...
            <a href="<?= URL_ROOT ?>/users" class="text-xs font-black underline">TUTUP</a>
        </div>
    </div>
    <?php endif; ?>
        ...
        <form method="GET" class="flex flex-col md:flex-row gap-6">
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" 
                       class="w-full pl-14 pr-6 py-4 bg-slate-50 border-b-2 border-slate-200 focus:border-kebana-blue focus:bg-white outline-none text-sm font-bold uppercase transition-all"
            ...
            <?php if ($search): ?>
            <a href="<?= URL_ROOT ?>/users" class="px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-widest flex items-center hover:text-red-500">
                KOSONGKAN
            </a>
            <?php endif; ?>
        </form>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b-2 border-slate-200">
                        <th class="px-8 py-5 text-xs font-black text-kebana-blue uppercase tracking-[0.2em]">ID</th>
                        <th class="px-8 py-5 text-xs font-black text-kebana-blue uppercase tracking-[0.2em]">MAKLUMAT PENGGUNA</th>
                        <th class="px-8 py-5 text-xs font-black text-kebana-blue uppercase tracking-[0.2em]">PERANAN</th>
                        <th class="px-8 py-5 text-xs font-black text-kebana-blue uppercase tracking-[0.2em]">CAWANGAN</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr>
                        <td colspan="6" class="px-8 py-24 text-center">
                            <i class="fa-solid fa-users-slash text-5xl text-slate-200 mb-6 block"></i>
                            <p class="text-xs font-black text-slate-400 uppercase tracking-[0.3em]">Tiada Rekod Pengguna Ditemui</p>
                        </td>
                    </tr>
                    <?php else: ?>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-8 py-6">
                            <?php 
                            $role_color = 'bg-slate-100 text-slate-700';
                            ...
                            ?>
                            <span class="inline-block px-3 py-1 text-xs font-black uppercase tracking-widest <?php echo $role_color; ?>">
                        ...
                        <td class="px-8 py-6 text-xs font-bold text-slate-600 italic">
                            <?php 
                        ...
                        <td class="px-8 py-6 text-right space-x-3">
                            <a href="<?= URL_ROOT ?>/users/edit/<?php echo $u['user_id']; ?>" class="inline-flex items-center justify-center w-10 h-10 bg-slate-50 text-slate-400 hover:bg-kebana-blue hover:text-white transition-all shadow-sm" title="Kemaskini">
